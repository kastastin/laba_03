...
class Element:
    ...
    def __init__(self, name = None, delay_mean = 1., delay_dev = 0., distribution = '', p = None, n_channel = 1):
        ...
        self.next_patient_type = None # тип пацієнта
        
    # розрахунок часової затримки
    def get_delay(self):
        if self.name == 'Reception Department':
            if self.next_patient_type == 1:
                self.delay_mean = 15
            elif self.next_patient_type == 2:
                self.delay_mean = 40
            elif self.next_patient_type == 3:
                self.delay_mean = 30
